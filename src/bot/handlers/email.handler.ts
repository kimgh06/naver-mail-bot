...
export class EmailHandler extends BaseHandler {
  ...
  async process(message: Message, args: string[]) {
    ...
    const acc_url = `https://auth.worksmobile.com/oauth2/v2.0/authorize?client_id=${process.env.NAVER_CLIENT}&redirect_uri=${process.env.REDIRECT}&scope=openid&response_type=code&state=UmyR2sX9gO`
